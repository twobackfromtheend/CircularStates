...
# Associate each i with a t.
t_list = np.linspace(0, 1000, steps)  # ns

for _i, dc_field in enumerate(tqdm(dc_fields)):
    ...
    # hamiltonian_with_rf = hamiltonian + e_rf * mat_2_combination
    t = t_list[_i]
    e_rf_t = e_rf * np.cos(rf_freq * t)
    print(e_rf_t, t, rf_freq * t)
    hamiltonian_with_rf = np.diagflat(np.diagonal(hamiltonian)) + e_rf_t * mat_2_combination

    eigenvalues, eigenvectors = np.linalg.eigh(hamiltonian_with_rf)
    energies_with_rf.append(eigenvalues - hamiltonian[s, s])
...
save_current_fig('avoided_crossings_full')
