...
def generate_matrices(n: int, stark_map: arc.StarkMap, s=0.5, with_zeeman_energy_shift: bool = False):
    """
    ...
    :param stark_map:
    :param s:
    :param with_zeeman_energy_shift:
    :return:
    """
    global wignerPrecal
    wignerPrecal = True
    ...
    for ii in range(dimension):
        ...
        if with_zeeman_energy_shift:
            ...
            mat_1[ii][ii] = atom_energy + zeeman_energy_shift
        else:
...
def load_hamiltonian(name: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray,]:
    ...
    with np.load(file) as data:
        return data['mat_1'], data['mat_2'], data['mat_2_minus'], data['mat_2_plus']
...
if __name__ == '__main__':
    ...
    np.savez_compressed(
        ...
        mat_1=matrices[0],
        mat_2=matrices[1],
        mat_2_minus=matrices[2],
        mat_2_plus=matrices[3],
    )
